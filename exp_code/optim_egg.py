...
if __name__=="__main__":
    ...
    for it in loop:
        apply_transformation(params, opt)
        img_hdr = mi.render(scene, params, seed=it, spp=spp, integrator=integrator, sensor=0)
        img_ldr = np.array(mi.util.convert_to_bitmap(img_hdr[...,:3]))
        mi.util.write_bitmap(f"tmp/optim_{exp_name}_{method}_{loss_type}_{spp}_{it}.png", img_hdr[...,:3])
        if log_level>0:
        ...
        if loss_type=="RGBXY" and method=="dpm":
            ...
            grad_ = matcher.match_Sinkhorn(img_ldr_low[...,:3].reshape(-1,3), gt_img_ldr_low[...,:3].reshape(-1,3))
    ...
    img_final_hdr = mi.render(scene, params, seed=0, spp=gt_spp, sensor=0, integrator=integrator)
